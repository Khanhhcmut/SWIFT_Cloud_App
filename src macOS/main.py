...
class MainWindow(QWidget):
    ...
    def __init__(self, token=None, storage_url=None, login_window=None, dicom_enabled=True):
        ...
        logout_btn.setStyleSheet("color: white; background-color: transparent; border: none;")
        help_btn.setStyleSheet("color: white; background-color: transparent; border: none;")
        ...
        def on_accept():
            if user_manual_radio.isChecked():
                ...
                admin_pass, ok = QInputDialog.getText(
                    self,
                    "Admin Confirmation",
                    "Enter admin password to change storage limit:",
                    QLineEdit.Password
                )
                ...
                if admin_pass != "change":
                    QMessageBox.critical(self, "Incorrect Password", "Admin password is incorrect.")
                    return
                ...
                if ok:
                    try:
                        ...
                        path = user_json_path("config.json")

                        if os.path.exists(path):
                            cfg = secure_json_load(path)

                        if "users" not in cfg:
                            cfg["users"] = {}
                        cfg["users"][username] = {"quota_gb": new_quota}

                        secure_json_dump(cfg, path)
        ...
        def on_accept():
            ...
            try:
                ...
                if response.status_code == 204:
                    try:
                        path = user_json_path("saved_users.json")
                        if os.path.exists(path):
...
#Lưu thông tin login và switch user
    # Đảm bảo phần limit được đồng nhất trong app
    def ensure_config_container(self):
        try:
            config_url = f"{self.storage_url}/config"
            headers = {"X-Auth-Token": self.token}
            resp = requests.put(config_url, headers=headers)
            return resp.status_code in (201, 202)
        except:
            return False

    def download_remote_config(self):
        try:
            file_url = f"{self.storage_url}/config/config.json"
            headers = {"X-Auth-Token": self.token}
    ...
    def upload_remote_config(self):
        try:
            ...
            resp = requests.put(file_url, headers=headers, data=data)
            return resp.status_code in (201, 202)
    ...
    def switch_saved_user(self, index):
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Switch user",
                f"Switched to user {user['user_display']} successfully"
            )
    ...
    def delete_selected_user(self):
        if not self.saved_users:
            QMessageBox.information(self, "Delete User", "No saved users available.")
            return

        dialog = QDialog(self)
        dialog.setWindowTitle("Delete saved user")
        dialog.setMinimumWidth(400)
        layout = QVBoxLayout(dialog)

        # Label
        layout.addWidget(QLabel("Select a user to delete:"))

        # Group radio buttons
        radio_group = QGroupBox()
        form_layout = QFormLayout(radio_group)
        radio_buttons = []
        for user in self.saved_users:
            rb = QRadioButton(user["user_display"])
            radio_buttons.append(rb)
            form_layout.addRow(rb)
        radio_buttons[0].setChecked(True)  # chọn mặc định user đầu tiên
        layout.addWidget(radio_group)

        # Password input
        pass_edit = QLineEdit()
        pass_edit.setEchoMode(QLineEdit.Password)
        pass_edit.setPlaceholderText("Enter password of selected user")
        ...
        def on_accept():
            ...
            for i, rb in enumerate(radio_buttons):
                if rb.isChecked():
                    selected_user = self.saved_users[i]
                    break
            ...
            entered_pass = pass_edit.text()
            if entered_pass != selected_user.get("password"):
                QMessageBox.critical(dialog, "Incorrect Password", "The password is incorrect. Cannot delete user.")
                return
            ...
            confirm = QMessageBox.question(
                self,
                "Confirm Delete",
                f"Delete user '{selected_user['user_display']}'?",
                QMessageBox.Yes | QMessageBox.No
            )
            if confirm == QMessageBox.Yes:
                self.saved_users.remove(selected_user)
                secure_json_dump(self.saved_users, user_json_path("saved_users.json"))
                self.load_saved_users()

                QMessageBox.information(self, "User Deleted", f"User '{selected_user['user_display']}' has been deleted.")
                dialog.accept()

                # Nếu xóa user hiện tại thì logout
                if selected_user["user_display"] == self.saved_users[self.current_user_index].get("user_display", ""):
                    QTimer.singleShot(2000, lambda: self.logout(skip_confirm=True))
...
#Xử lý folder
    ...
    def handle_drop_to_container_table(self, event):
        ...
        system_containers = {"backup", "dicom", "config"}
    ...
    def create_container_dialog(self):
        ...
        if ok and name:
            ...
            reserved_names = ["backup", "dicom", "config"]
